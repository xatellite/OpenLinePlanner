...
pub struct Streets {
    ...
    #[serde(serialize_with = "Streets::serialize_streetgraph", deserialize_with = "Streets::deserialize_streetgraph")]
    pub streetgraph: UnGraphMap<NodeId, f64>,
}

impl Streets {
    fn serialize_streetgraph<S>(graph: &UnGraphMap<NodeId, f64>, serializer: S) -> Result<S::Ok, S::Error> where S: serde::Serializer {
        let edges: Vec<(NodeId, NodeId, f64)> = graph.all_edges().into_iter().map(|edge| (edge.0, edge.1, *edge.2)).collect();
